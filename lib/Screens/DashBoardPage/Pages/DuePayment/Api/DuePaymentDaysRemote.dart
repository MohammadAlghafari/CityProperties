...
abstract class DuePaymentDataSource {
  Future<Either<String, DuePaymentModel>> getDuePayment();
}

class  DuePaymentDataSourceImpl extends DuePaymentDataSource {
  final Dio dio;
  final DataConnectionChecker networkInfo;

  DuePaymentDataSourceImpl({required this.dio, required this.networkInfo});

  @override
  Future<Either<String, DuePaymentModel>> getDuePayment() async {
    if (await networkInfo.hasConnection) {
      try {
        String? tenantCode =Preferences.preferences!.getString(KeyConstants.keyUserId);
        //tenantCode for test 11417 13736 9
        final re = await dio.post(
          AppConstants.duePaymentDays,
          queryParameters: {
        ...
        return Right(DuePaymentModel.fromJson(json.decode(re.data)));
